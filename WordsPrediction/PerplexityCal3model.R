...
set.seed(1000)
trainsize <- 0.01   # Do not train on the whole cross validation
...
crossvalidation <- function(fraction=0.1) {
      
res <- NULL
for (m in 1:length(disc)) {
      for (n in 1:length(disc)) {
            for (p in 1:length(disc)) {
                  fourdisc <- disc[m]
                  tridisc <- disc[n]
                  bidisc <- disc[p]
                  numtime <- floor(length(crosstraindata)*fraction)
                  pp <- val(numtime, fourdisc, tridisc, bidisc)
                  print(pp)
                  print(c(m,n,p))
                  
                  res <- rbind(res,c(fourdisc,tridisc,bidisc,pp))
                  save(res,file = "./dev4grams.RData")
...
val <- function(numtime,fourdisc =0 ,tridisc = 0 , bidisc = 0) {
      perplexity <- NULL
for (i in 1:numtime) {
      pbl <- NULL
      ...
      for (j in 1:(length(sentence)-1)){
            ...
            if (j == 1) {
                  input <- sentence[j]
                  pre <- predictword(input,fourdisc ,tridisc , bidisc)
                  real <- pre[n==nextword]
                  if (is.na(real$n[1])) {
                        pbl[j] <- NA
                        print("Unknown word")
                        # Add unknown word to dictionary
                        traintk1g[length(traintk1g)+1]$n <- nextword
                        traintk1g[length(traintk1g)+1]$freq <- 1
                        
                  }
                  else pbl[j] <- log(real$prob)
            }
            else if (j == 2) {
      ...
      pbl <- pbl[!is.na(pbl)]
      perplexity[i] <- (1/exp(sum(pbl)))^(1/(length(pbl)+1))
      if (perplexity[i] == Inf) perplexity[i] <- NA
      
      print(c(i,perplexity[i]))
 
}
      
      return(mean(perplexity, na.rm = TRUE))
}
